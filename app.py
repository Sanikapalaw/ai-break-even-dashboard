...
with col_in2:
    ...
    fixed_overhead = st.number_input("Fixed Rent/Admin ($)", value=25000.0)
...
# ----------------- 4. EXECUTIVE TABS -----------------
tab1, tab2, tab3, tab4 = st.tabs(["📊 Performance", "📈 Projections", "💰 Valuation", "🤖 AI Advisor"])
...
with tab1:
    ...
    with col_liq:
        # Cash Depletion Forecast (Using the forecast_months input)
        timeline = list(range(forecast_months + 1))
        cash_vals = [max(0, current_cash + (net_profit * m)) for m in timeline]
        fig_liq = px.area(x=timeline, y=cash_vals, title=f"{selected_horizon} Cash Forecast", labels={'x': 'Months', 'y': 'Cash Balance ($)'})
        fig_liq.add_hline(y=0, line_dash="dash", line_color="red")
        st.plotly_chart(fig_liq, use_container_width=True)

# --- TAB 2: PROJECTIONS (Using the forecast_months input) ---
with tab2:
    ...
    st.plotly_chart(px.line(df_p, x="Month", y=["Revenue", "Profit"], title="Operational Performance Path"), use_container_width=True)
...
with tab3:
    ...
    if company_stage == "Idea":
        ...
        val_method, valuation = "Revenue Multiple (3x)", (revenue * 12) * 3
    else:
        val_method, valuation = "DCF Model", (net_profit * 12) / 0.15 

    st.metric(f"Valuation ({val_method})", f"${valuation:,.2f}")
    st.info(f"Methodology: Based on the {company_stage} stage and current profitability.")

# --- TAB 4: AI ADVISOR ---
with tab4:
    if st.button("Generate Executive Briefing"):
        if api_key:
            prompt = f"CFO Analysis: Stage {company_stage}, Profit {net_profit}, Runway {runway}. Provide 3 executive steps."
            try:
